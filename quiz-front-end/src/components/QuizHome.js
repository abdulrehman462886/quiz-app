    ...
    {props.inputarr.map((value, index) => {
            ...
            return (
              ...
              <h4><strong>{index + 1}: </strong>
                ...
                <h5><input type="radio" id="textA"  name="text" value={value.a} onChange={props.updateoption} onClick={props.onradioclick}/> <label htmlFor="textA">{value.a}</label><br></br>
                <input type="radio" id="textB" name="text" value={value.b} onChange={props.updateoption} onClick={props.onradioclick}/> <label htmlFor="textB">{value.b}</label><br></br>
                <input type="radio" id="textC" name="text" value={value.c} onChange={props.updateoption} onClick={props.onradioclick}/> <label htmlFor="textC">{value.c}</label><br></br>
                <input type="radio" id="textD" name="text" value={value.d} onChange={props.updateoption} onClick={props.onradioclick}/> <label htmlFor="textD">{value.d}</label><br></br></h5>
                <button  type="button" className="btn btn-primary my-5" onClick={() => {props.savehandle(value.e,index)}} disabled={props.bol1}>Save</button>
              </div>
            )}
    })[props.qindex]}
    ...
    {props.data.map((obj, index) => { 
           ...
           <strong> <span>&#8594;</span> Correct: </strong> <label style={{color:"green" , fontFamily:"fantasy"}}>{obj.value.e}</label><br></br> 
            ...
            return (
              ...
              <h5><strong>{index + 1}: </strong>
                <strong>{obj.value.statement}</strong></h5>
                <h5><label htmlFor="textA">{obj.value.a}</label><br></br>
                <label htmlFor="textB">{obj.value.b}</label><br></br>
                <label htmlFor="textC">{obj.value.c}</label><br></br>
                <label htmlFor="textD">{obj.value.d}</label><br></br>
                <strong> <span>&#8594;</span> Correct: </strong> <label style={{color:"green" , fontFamily:"fantasy"}}>{obj.value.e}</label><br></br> 
                <strong> <span>&#8594;</span> Selected: </strong> <label style={{color: obj.value.e !== obj.selected.option ? "red" : "green", fontFamily:"fantasy"}}>{obj.selected.option}</label></h5>
                <hr></hr>
              </div>
